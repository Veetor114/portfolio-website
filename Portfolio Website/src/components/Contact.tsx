...
export function Contact() {
  ...
  const MAKE_WEBHOOK_URL = "https://hook.eu2.make.com/ov9ltcdtpr9ffm3baeaz4wt2tbp533ph"; // <-- Replace with your Make.com webhook URL
            ...
            <Card className="shadow-lg border-0 dark:bg-gray-800">
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <Button type="submit" className="w-full bg-green-600 hover:bg-green-700 disabled:opacity-50" disabled={isSubmitting}>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="space-y-8">
            <Card className="shadow-lg border-0 dark:bg-gray-800">
              ...
              <CardContent className="space-y-6">
                {contactInfo.map((info, index) => {
                  ...
                  return info.href ? <a key={index} href={info.href} className="block">{content}</a> : <div key={index}>{content}</div>;
                })}
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-3">
                  {socialLinks.map((social, index) => {
                    ...
                    return (
                      <Button key={index} variant="outline" size="lg" className="flex-1 border-green-200 text-green-700 hover:bg-green-50 dark:border-green-800 dark:text-green-400 dark:hover:bg-green-900/20" onClick={() => window.open(social.href, "_blank")}>
                ...
                <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={handleDownloadCV}>
            ...
            <div className="text-center p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
              ...
              <p className="text-gray-600 dark:text-gray-300">
                I typically respond to emails within 24-48 hours. For urgent matters, feel free to reach out via LinkedIn or Email for a faster response.
              </p>
            </div>
          </motion.div>
        </div>
